...
应用采用紧凑的 **IDE 工作台布局**，烧录、RTT、串口、蓝牙和调试共用同一套界面结构：

右侧配置检查器可通过顶栏按钮收起或展开，拖动检查器左侧分隔线可以调整宽度。工作流尚未连接或没有数据时，主区域的“打开配置”按钮会直接展开并聚焦对应配置。底部输出日志默认折叠，需要时可从日志栏展开。
