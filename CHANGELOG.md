...
- 新增通用二进制协议设计器：支持固定帧头、固定长度/长度字段/帧尾分帧、多消息类型匹配，以及整数、浮点、BCD、位域和固定数组字段
- 新增 SUM8/SUM16/XOR8、CRC8/16/32 校验配置与常用 CRC 预设，并提供 HEX 实时预览、协议 JSON 导入和导出
- 通用二进制协议接入串口、TCP/UDP、RTT、BLE 和经典蓝牙 SPP 原始字节流，解析字段可直接复用图表、滤波、FFT、控制面板、触发器和导出链路
- 新增本机协议库，可按名称保存、覆盖、删除多个协议，并在数据解析页快速切换

### 优化

- 消息字段页补充消息类型与默认消息说明，新增消息后保持当前编辑项，不再被实时样本刷新重置

## [2.9.0] - 2026-08-14

### 新增

- 串口新增 CAN / SLCAN 解析：支持经典 CAN 与 CAN FD 帧列表、payload 位域信号映射、总线负载历史、按 ID 分布、阈值告警与 CSV 导出
- CAN 工作区新增 DBC 导入、信号节点映射、适配器自动初始化，以及经典 CAN / CAN FD 帧发送

### 优化

- 优化串口数据解析检查器的窄侧栏布局，CAN 与 Modbus 参数按可用宽度自动换列，避免标签、输入框和开关被挤压

## [2.8.1] - 2026-08-13

### 修复

- 修复 Windows 本地串口发送文件时，短读取超时同时限制写入、导致大分块可能报“信号灯超时时间已到（os error 121）”的问题
...
- 串口右侧“发送设置”整合为紧凑的“收发设置”，接收与发送可分别配置文本/HEX 和编码
- 优化串口与 RTT 日志复制：当前视图独立处理全选，复制后清除高亮，右键未选中行可直接复制
- 右键菜单改为不透明系统层显示，并限制在窗口边界内
- 桌面端日志与终端复制改用原生系统剪贴板，支持 Windows `Win+V` 剪贴板历史

### 修复

- 修复串口接收始终按 UTF-8 解码、设备输出 GBK 中文时显示乱码的问题
- 修复串口选择 GBK 发送时实际仍按 UTF-8 编码的问题
- 修复日志区 `Ctrl+A` 可能选中工具栏、侧栏等整个页面的问题
- 修复右键复制菜单背景过于透明的问题

### 内部

- 统一 Rust 代码格式
- 新增 GBK 接收分片与发送字节回归检查
