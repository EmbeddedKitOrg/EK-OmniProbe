...
- 串口控制面板支持提升为独立窗口，并可从主窗口定位或收回
- 控制面板新增串口日志组件，可配置显示全部、接收或发送日志

### 优化

- 控制面板改用自由画布，组件支持任意位置拖动和连续尺寸调整，不再吸附网格
- 控制面板配置升级为 `version: 4`，现有 `version: 3` 网格布局可自动迁移
- RTT、串口和 BLE 的时域波形支持直线或平滑曲线连接，控制面板 YT 波形可独立选择
