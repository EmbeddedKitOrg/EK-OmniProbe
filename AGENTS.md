...
- 涉及页面、组件、布局或交互变化时，必须使用 Playwright 完成 UI 验收，不使用 `computer-use` 代替。
- 至少覆盖桌面和窄屏两种视口，检查关键操作、文本溢出、控件遮挡、页面滚动和响应式布局。
- 验收时保存截图，并检查浏览器控制台错误和页面异常；临时截图与测试产物不得提交到仓库。
- 优先复用仓库已有 Playwright 配置和浏览器缓存；项目未安装时先说明所需依赖，再决定是否安装。
