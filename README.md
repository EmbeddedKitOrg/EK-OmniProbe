...
<p align="center">
  面向嵌入式开发者的一体化桌面调试工作台：烧录、源码调试、RTT、串口和蓝牙，一个应用完成。
</p>
...
EK-OmniProbe 把嵌入式开发中经常分散在多个软件里的工作流放进同一个桌面应用。你可以连接调试探针烧录和调试固件，也可以使用 RTT、串口或 BLE 查看日志、发送命令、绘制波形和分析 FFT。
...
串口时域波形支持粘贴 MATLAB 生成的 FIR 系数或 IIR 的 SOS/ScaleValues，实时叠加比较原始与滤波结果；滤波不会覆盖日志、CSV、FFT 和 AI 使用的原始数据。详见 [MATLAB 参数滤波](docs/MATLAB_FILTER_GUIDE.md)。
