...
### 最新版本 v0.9.0 (2026-03-31)

#### 新增与改进
- ✨ **工作台体验升级** - 参考 Entrance 的信息架构重整 RTT / 串口工作台，让界面更像完整桌面工作区
- ✨ **波形 / FFT 入口前移** - RTT 和串口工具栏都提供显式 `波形` / `FFT` 快捷入口，进入分析更直接
- ✨ **设置中心** - 新增统一的设置入口，支持主题切换、默认工作台、RTT / 串口默认视图和默认波形域偏好
- ✨ **日志面板偏好** - 日志区域高度支持持久化，工作台重新打开后可恢复上次习惯

#### 界面收口
- 🔧 **图表摘要增强** - 图表区补充活跃系列、采样信息、解析健康度和最近数据快照
- 🔧 **工作流提示降噪** - 精简 RTT / 串口顶部说明和面板提示，减少信息堆叠，把首屏空间还给主图表
- 🔧 **工具栏分组统一** - RTT / 串口操作按连接、采集、查看、分析分组，工作流更一致

#### 延续优化
- 🚀 **虚拟化列表与状态管理优化** - 大数据场景下继续保持 RTT / 串口查看流畅
- 🚀 **图表链路统一** - RTT 与串口复用同一套波形、FFT 和图表分析能力
