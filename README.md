...
**使用步骤**：
1. 启动 RTT 后点击 `智能启用` 或 `配置图表`
2. 选择 `波形示波器` 或其他图表类型
3. 也可以直接点击 RTT / 串口工具栏里的 `波形` 或 `FFT` 快捷入口
4. 查看实时更新的图表
5. 在波形模式下使用 `Time / FFT` 切换时域与频域
6. 使用缩放、拖动、悬停查看细节和统计信息
...
- 🚀 **Zustand 精确选择器** - 避免无关状态变化触发重渲染，全面提升响应速度
- 🚀 **条件渲染优化** - 非活跃模式停止执行 hooks 和事件监听，减少后台资源消耗
- 🚀 **修复事件监听器频繁重建** - 修复 useEffect 依赖问题，Tauri 事件监听器不再频繁重建

#### 重构
- 🔧 **抽取 storage.ts** - 统一 localStorage 读写，消除重复 try/catch 模式
- 🔧 **抽取 ansiParser.ts / formatters.ts** - 公共模块化，消除 ANSI 解析和格式化函数重复
- 🔧 **统一 parseLogLevel** - 移至 utils.ts，消除 rttStore 和 serialStore 的重复定义
- 🔧 **通用 ChartViewer 组件** - 重构 SerialChartViewer，修复其错误使用 RttStore 的问题
- 🔧 **代码清理** - 移除 console.log、删除废弃导出、修复 any 类型

#### 修复
- 🐛 **固件格式校验** - `verify_firmware` 添加 ELF/HEX 格式检测，非 BIN 文件返回明确错误
- 🐛 **统一锁实现** - 使用 `parking_lot::Mutex`，移除 `lazy_static` 依赖，消除锁中毒 panic 风险
- 🐛 **内存操作安全校验** - 为 `read_memory`/`read_flash`/`erase_sector` 添加大小上限校验
- 🐛 **TCP 完整写入** - `TcpSerial::write` 改用 `write_all`，避免 TCP 部分写入
- 🐛 **更新进度条** - 修复显示单个 chunk 比例而非累计进度的 bug
- 🐛 **移除重复 Download 按钮** - 清理 FlashToolbar 中功能重复的操作项
