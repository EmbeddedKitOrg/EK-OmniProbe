...
- 支持本地串口（COM 口）、TCP 远程串口（ser2net、ESP-Link）和双向 UDP 数据接口
- 同时提供日志视图和终端视图，支持收发分屏、文本/HEX 切换
- 复用 RTT 的颜色解析、波形示波器和 FFT 频谱功能
- 发送历史记录、终端本地回显和常用控制键快捷发送
- 日志三档复制：`Ctrl+C` 纯文本 / `Ctrl+Shift+C` 含时间戳和方向 / 右键弹菜单临时切换，并可将当前输出窗口完整保存为 TXT
...
- **IDE 风格界面**：左侧模式轨道、顶部命令栏、中央工作区、可折叠和拖宽的右侧配置检查器与默认折叠的底部日志
...
- **关于作者入口**：设置中心的“工具”页可查看左岚信息、Bilibili 和项目 GitHub
...
1. 启动 RTT 后在工具栏 `更多` 中点击 `智能启用`，或打开 `图表配置`
2. 选择 `波形示波器` 或其他图表类型
3. 也可以从 RTT / 串口工具栏的 `更多` 中点击 `波形` 或 `FFT`
4. 查看实时更新的图表
...
> 💡 在 RTT 模式工具栏的「**更多**」中点击「**接入指南**」，可以直接在应用内看到完整的代码示例与常见坑。
...
1. 在侧边栏选择“UDP 数据接口”
2. 配置本地绑定地址/端口和远端地址/端口
3. 点击“连接”后即可使用现有日志、终端和发送栏双向收发 UDP 数据
...
- **波形 / FFT 入口**：可从串口工具栏的 `更多` 菜单切入时域或频域分析
- **AI 数据桥接**：将图表解析后的标准样本通过本机 TCP/NDJSON 提供给 AI，支持只读分析、显式授权后的安全串口调参，以及一键复制 Skill 链接和安装文案
- **更多菜单**：日志视图可集中配置收发分屏、时间戳、`RX/TX` 前缀、文本/HEX；终端视图可集中配置本地回显和控制键拦截
- **右键保存**：在串口、RTT、BLE、应用日志或调试输出区域右键，可将当前窗口全部内容保存为 UTF-8 TXT 文件
...
AI 数据桥接、客户端命令和安全调参流程详见 **[AI 数据桥接与可视化调参指南](docs/AI_TUNING_GUIDE.md)**。

### 蓝牙模式使用

蓝牙模式提供两种工作方式，由右侧配置检查器顶部的「工作模式」区域切换：
...
1. 切换到蓝牙模式（左侧模式轨道「蓝牙」按钮或 Ctrl+4）。蓝牙模式不依赖调试探针。
2. 在右侧「设备扫描」区域点击「扫描」，等待 6 秒后会列出发现的设备（按是否有名称、信号强度排序）。
...
- 点击「开始接收 (Notify)」后底部会持续刷新 BLE 数据；可在工具栏切换 `仅文本 / 分屏 / 仅图表`，或从 `更多` 进入 `波形 / FFT`。
...
1. 在系统蓝牙设置中和目标设备完成配对
2. 在蓝牙模式右侧配置检查器的「工作模式」选择 `经典蓝牙 SPP`
...
最新版本 v1.5.0。完整版本历史与升级说明请查看 [CHANGELOG.md](CHANGELOG.md)。
